...
#Copies all the blocks if the player is facing north

scoreboard players remove shift_y msb.vol 1
scoreboard players operation shift_x msb.vol = x msb.vol
function msb:break/loops/copy/north/left
execute if score shift_y msb.vol matches 0.. positioned ~ ~1 ~ run function msb:break/loops/copy/north/up